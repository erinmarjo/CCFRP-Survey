...
ss911dif <- bind_cols(ss9dif, ss11dif)%>%
  ...
  ggplot(aes(x = option9, y = per_dif, label = paste0(per_dif, "%"), fill=option9))+
  geom_col(position = "dodge", color="black")+
  scale_fill_viridis_d(option="mako",labels=beforeafter, direction = -1)+
...
ss1012graph <- ss1012df %>%
  ...
  labs(x = "", y = 'Percent\n') +
  theme_bw() + 
  ggtitle("Did you believe the creation of MPAs would affect \n the size/abundance of groundfish inside MPAs? \n \n Before and After Volunteering with CCFRP")+
  theme(panel.grid = element_blank(),
        axis.title=element_text(size=12),
        legend.title = element_blank(),
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
...
ss1012change <- dfnew %>% 
  ggplot(aes(x = q12, y = change, label = paste0(round(change, digits = 1), "%"), fill=q12))+
  geom_col(position = "dodge", color="black")+
  scale_fill_viridis_d(option="mako",labels=beforeafter_groundfish, direction = -1, " ")+
...
ss13 <- survey_dat_clean %>% 
  ...
  geom_col(position = "dodge", fill="#0570b0", color="black")+
  geom_text(position = position_dodge(width = 0.9),
            vjust = -0.5,
            size = 4)+
  ...
  ggtitle("If you believe that California MPA creation affected groundfish abundance and/or size, \n what aspect(s) of MPAs do you believe caused these changes?")+
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
...
ss14 <- survey_dat_clean %>% 
  ...
  labs(x = "", y = 'Percent\n') +
  theme_bw() + 
  ggtitle("In your opinion, what is the purpose of a Marine Protected Area?")+
  theme(panel.grid = element_blank(),
        legend.position = "none",
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        axis.title=element_text(size=12),
...
ss16a <- survey_dat_clean %>% select(q16) %>% drop_na() %>% count(q16 = factor(q16))%>% 
  ...
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  ggtitle("Have you experienced a difference in fishing in a MPA versus outside of an MPA? \n \n Higher Abundance")+
  theme(panel.grid = element_blank(),
         plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        legend.position = "none", 
        axis.title=element_text(size=12),
...
ss17 <- survey_dat_clean %>% select(q19) %>% drop_na() %>% count(q19 = factor(q19))%>% 
  ...
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  ggtitle("What is the primary reason you enjoy MPA fishing with CCFRP?")+
  theme(panel.grid = element_blank(),
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        legend.position = "none", 
        axis.title=element_text(size=12),
...
ss18 <- survey_dat_clean %>% select(q20) %>% drop_na() %>% count(q20 = factor(q20))%>% 
  ...
  ggtitle("How Much Would You Pay to Fish in an MPA?")+
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        legend.position = "none", 
...
ss23 <- survey_dat_clean %>% select(q26) %>% drop_na() %>% count(q26 = factor(q26))%>% 
  ...
  ggtitle("How Well Managed are California Fisheries?")+
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        legend.position = "none", 
...
ss24 <- survey_dat_clean%>%
  ...
  coord_flip()+
  theme_bw()+
  ggtitle("Which fishing gear or fishing practices have the greatest impact on \n the degradation of the marine environment or reduction in fish stocks?")+
  theme(panel.grid = element_blank(),
        legend.position = "none",
         plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        axis.title = element_text(size = 12),
...
ss25 <- survey_dat_clean %>%
  ...
  ggtitle("Are these issues a threat to California's marine environment?")+
  scale_fill_brewer(palette = "BrBG", "Opinion")+
  coord_flip()+
  theme_bw()+
 theme(panel.grid = element_blank(),
         plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
...
survey_dat_filter<-survey_dat_clean%>%
  select(responseid,q1,q2,q3,q4,q30,q9,q10,q11,q12, q13, q16, q17, q18, q26, q32)
#survey_dat_filter$q1 <- recode(survey_dat_filter$q1, 20118 = 2018)

#Recode Region, demographic unformation and participation information for further analysis. 

#Region
survey_dat_filter$Region<-survey_dat_filter$q4
survey_dat_filter$Region<-as.factor(survey_dat_filter$Region)
survey_dat_filter$Region<-recode_factor(survey_dat_filter$Region, `1` = "North", `2` ="North", `3` ="Central", `4` = "Central", `5` = "South", `6` ="South")
                  
#Age Group
survey_dat_filter$Age<-survey_dat_filter$q32
survey_dat_filter$Age<-as.factor(survey_dat_filter$Age)
survey_dat_filter$Age<-recode_factor(survey_dat_filter$Age, `1` = "18-34", `2` ="18-34", `3` ="35-54", `4` = "35-54", `5` = "55+", `6` ="55+")


#Frequency of Participation
survey_dat_filter$Freq<-survey_dat_filter$q2
survey_dat_filter$Freq<-as.factor(survey_dat_filter$Freq)
survey_dat_filter$Freq<-recode_factor(survey_dat_filter$Freq, `1` = "Every Year", `2` ="Every Few Years", `3` ="Infrequently (1-2 years only)", `4` = "Never")

#Number of Trips
survey_dat_filter$Trips<-survey_dat_filter$q2
survey_dat_filter$Trips<-as.factor(survey_dat_filter$Trips)
survey_dat_filter$Trips<-recode_factor(survey_dat_filter$Trips, `1` = "1", `2` ="2-10", `3` ="2-10", `4` = "11+", `5`="20+")
...
Icount<-survey_dat_filter%>%select(q4)%>%drop_na()%>%
  group_by(q4)%>%
  summarize(count=n())
# Start examining question 9 in relation to the year of initial participation
...
Recode institution as a managment region: North, Central, or South to examine change in perception based on region of sampling. 

```{r}
q9.R<-survey_dat_filter%>%select(Region, q9)%>%drop_na()%>%
  group_by(Region)%>%
  count(q9 = factor(q9))%>%
  spread(q9, n)
...
beforeR<-ggplot(q9.R, aes(x=q9, y=pct9, fill=Region, label = paste0(pct9, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
  scale_y_continuous(limits = c(0,85))+
...
# Start examining question 11 in relation to the year of initial participation
q11.R<-survey_dat_filter%>%select(Region, q11)%>%drop_na()%>%
  group_by(Region)%>%
  count(q11 = factor(q11))%>%
  spread(q11, n)
...
afterR<-ggplot(q11.R, aes(x=q11, y=pct11, fill=Region, label = paste0(pct11, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
  scale_y_continuous(limits = c(0,85))+
  scale_fill_manual(values=c("#2166ac","#fddbc7","#b2182b", "Region"))+
...
#Group the question 9 and 11 to calculate the % change after working with CCFRP.
q911.R<-left_join(q9.R, q11.R,by="Region")%>%
  filter(q9==q11)
q911.R$change<-(q911.R$pct11-q911.R$pct9)


changeR<-ggplot(q911.R, aes(x=q11, y=change, fill=Region, label = paste0(change, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
 # scale_y_continuous(limits = c(0,85))+
  labs(x = '', y = 'Percent\n') +
...
beforeR/afterR/changeR
```


Examine the Magnitude of Change in perception of MPAs after working with CCFRP all combined and by region

```{r}
xx<-survey_dat_filter%>%select(responseid,Region, q9, q11)%>%drop_na()%>%
  filter(q9!="6" & q11!="6")%>%
  group_by(responseid, Region)
xx$q9<-as.numeric(xx$q9)
xx$q11<-as.numeric(xx$q11)


X1 <- c('Positive', 'Somewhat \npositive', 'Neutral', 'Somewhat \nnegative', 'Negative')

xx$change<-xx$q9-xx$q11
xx$q9<-as.factor(xx$q9)

#Overall Response
mag<-ggplot(xx, aes(x=change))+
  geom_bar(stat="count", aes(fill=q9), color="black")+
  theme_bw()+
  xlim(-4,4)+
  geom_vline(xintercept=0, linetype="dashed", size=0.5)+
  ylab("Number of respondents")+
  xlab("Magnitude of change in opinions of MPAs")+
  ggtitle("Direction of Individual Change in Opinion of MPAs")+
  scale_fill_viridis_d(option="mako", labels = X1, "Initial Response", direction = -1)+
  theme(panel.grid = element_blank(),
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        axis.title=element_text(size=12),
        axis.text = element_text(size = 12))+
...
#Response by Region
ggplot(xx, aes(x=change))+
  geom_bar(stat="count", aes(fill=q9), color="black")+
  theme_bw()+
  xlim(-4,4)+
  geom_vline(xintercept=0, linetype="dashed", size=0.5)+
  xlab("Magnitude of Change in Opinions of MPAs")+
  scale_fill_viridis_d(option="mako", labels = X1, "Initial Response", direction = -1)+facet_wrap(~Region)
#More Positive or less Positive 

ss911graph/ss911dif/mag


xx1<-survey_dat_filter%>%select(responseid,Age, q9, q11)%>%drop_na()%>%
  filter(q9!="6" & q11!="6")%>%
  group_by(responseid, Age)
xx1$q9<-as.numeric(xx1$q9)
xx1$q11<-as.numeric(xx1$q11)


X1 <- c('Positive', 'Somewhat \npositive', 'Neutral', 'Somewhat \nnegative', 'Negative')

xx1$change<-xx1$q9-xx1$q11
xx1$q9<-as.factor(xx1$q9)

#Response by Age Group
ggplot(xx1, aes(x=change))+
  geom_bar(stat="count", aes(fill=q9), color="black")+
  theme_bw()+
  xlim(-4,4)+
  geom_vline(xintercept=0, linetype="dashed", size=0.5)+
  xlab("Magnitude of Change in Opinions of MPAs")+
  scale_fill_viridis_d(option="mako", labels = X1, "Initial Response", direction = -1)+facet_wrap(~Age)

#Number of Trips

xx2<-survey_dat_filter%>%select(responseid,Trips, q9, q11)%>%drop_na()%>%
  filter(q9!="6" & q11!="6")%>%
  group_by(responseid, Trips)
xx2$q9<-as.numeric(xx2$q9)
xx2$q11<-as.numeric(xx2$q11)


X1 <- c('Positive', 'Somewhat \npositive', 'Neutral', 'Somewhat \nnegative', 'Negative')

xx2$change<-xx2$q9-xx2$q11
xx2$q9<-as.factor(xx2$q9)

#Response by Number of Trips
ggplot(xx2, aes(x=change))+
  geom_bar(stat="count", aes(fill=q9), color="black")+
  theme_bw()+
  xlim(-4,4)+
  geom_vline(xintercept=0, linetype="dashed", size=0.5)+
  xlab("Magnitude of Change in Opinions of MPAs")+
  scale_fill_viridis_d(option="mako", labels = X1, "Initial Response", direction = -1)+facet_wrap(~Trips)


###Frequency of Participation
xx3<-survey_dat_filter%>%select(responseid,Freq, q9, q11)%>%filter(Freq!="Never")%>%drop_na()%>%
  filter(q9!="6" & q11!="6")%>%
  group_by(responseid, Freq)
xx3$q9<-as.numeric(xx3$q9)
xx3$q11<-as.numeric(xx3$q11)


X1 <- c('Positive', 'Somewhat \npositive', 'Neutral', 'Somewhat \nnegative', 'Negative')

xx3$change<-xx3$q9-xx3$q11
xx3$q9<-as.factor(xx3$q9)

#Response by Age Group
ggplot(xx3, aes(x=change))+
  geom_bar(stat="count", aes(fill=q9), color="black")+
  theme_bw()+
  xlim(-4,4)+
  geom_vline(xintercept=0, linetype="dashed", size=0.5)+
  xlab("Magnitude of Change in Opinions of MPAs")+
  scale_fill_viridis_d(option="mako", labels = X1, "Initial Response", direction = -1)+facet_wrap(~Freq)

```

Examine change in perception of MPAs based on the frequency of sampling with CCFRP (q2)

```{r}
q9.F<-survey_dat_filter%>%select(Freq, q9)%>%drop_na()%>%filter(Freq!="Never")%>%
  group_by(Freq)%>%
  count(q9 = factor(q9))%>%
  spread(q9, n)
...
beforeF<-ggplot(q9.F, aes(x=q9, y=pct9, fill=Freq, label = paste0(pct9, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
  scale_y_continuous(limits = c(0,85))+
  scale_fill_brewer(palette = "BrBG")+
   labs(x = '', y = 'Percent\n') +
  theme_bw() +
  facet_wrap(~Freq, labeller=labeller)+
  geom_text(position = position_dodge(width = 1),
            vjust = -0.5,
            size = 3)+
  ggtitle("Opinion about MPAs Before Volunteering with CCFRP") +
...
# Start examining question 11 in relation to the year of initial participation
q11.F<-survey_dat_filter%>%select(Freq, q11)%>%drop_na()%>%filter(Freq!="Never")%>%
  group_by(Freq)%>%
  count(q11 = factor(q11))%>%
  spread(q11, n)
...
afterF<-ggplot(q11.F, aes(x=q11, y=pct11, fill=Freq, label = paste0(pct11, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
  scale_y_continuous(limits = c(0,85))+
scale_fill_brewer(palette = "BrBG")+
...
#Group the question 9 and 11 to calculate the % change after working with CCFRP.
q911.F<-left_join(q9.F, q11.F,by="Freq")%>%
  filter(q9==q11)
q911.F$change<-(q911.F$pct11-q911.F$pct9)


changeF<-ggplot(q911.F, aes(x=q11, y=change, fill=Freq, label = paste0(change, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
 # scale_y_continuous(limits = c(0,85))+
  labs(x = '', y = 'Percent\n') +
...
beforeF/afterF/changeF
```

Examine change in perception of MPAs based on the Number of Trips with CCFRP (q3)

```{r}
q9.T<-survey_dat_filter%>%select(Trips, q9)%>%drop_na()%>%
  group_by(Trips)%>%
  count(q9 = factor(q9))%>%
  spread(q9, n)
...
beforeT<-ggplot(q9.T, aes(x=q9, y=pct9, fill=Trips, label = paste0(pct9, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
  scale_y_continuous(limits = c(0,85))+
  scale_fill_brewer(palette = "BrBG")+
   labs(x = '', y = 'Percent\n') +
  theme_bw() +
  facet_wrap(~Trips, ncol=4, labeller=labeller)+
  geom_text(position = position_dodge(width = 1),
            vjust = -0.5,
            size = 3)+
  ggtitle("Opinion about MPAs Before Volunteering with CCFRP") +
...
# Start examining question 11 in relation to the year of initial participation
q11.T<-survey_dat_filter%>%select(Trips, q11)%>%drop_na()%>%
  group_by(Trips)%>%
  count(q11 = factor(q11))%>%
  spread(q11, n)
...
afterT<-ggplot(q11.T, aes(x=q11, y=pct11, fill=Trips, label = paste0(pct11, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
  scale_y_continuous(limits = c(0,85))+
scale_fill_brewer(palette = "BrBG")+
...
#Group the question 9 and 11 to calculate the % change after working with CCFRP.
q911.T<-left_join(q9.T, q11.T,by="Trips")%>%
  filter(q9==q11)
q911.T$change<-(q911.T$pct11-q911.T$pct9)


changeT<-ggplot(q911.T, aes(x=q11, y=change, fill=Trips, label = paste0(change, "%")))+
  geom_bar(stat="identity", position=position_dodge(1), color="black")+
  scale_x_discrete(labels = beforeafter)+
 # scale_y_continuous(limits = c(0,85))+
  labs(x = '', y = 'Percent\n') +
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.text = element_text(size = 12),
        legend.position = "none")+
  facet_wrap(~Trips, ncol=4)+
  theme(strip.text.x = element_text(size = 12))


beforeT/afterT/changeT
```


Examine responses to 16 a b and c (q16, q17 and q18) by region
...
ss16a <- survey_dat_filter %>% select(Region, q16) %>% drop_na() %>% group_by(Region)%>%count(q16 = factor(q16))%>% 
    mutate(pct = n/sum(n)*100,
           pct = round(pct, digits = 0))%>%
  ggplot(aes(x = q16, y = pct, label = paste0(pct, "%"), fill=Region))+
  geom_col(position = "dodge", color="black")+
  geom_text(position = position_dodge(width = 0.9),
            vjust = -0.5,
            size = 4)+
  ...
  facet_wrap(~Region)+
  labs(x = ' ', y = 'Percent\n') +
  ggtitle("Have you experienced a difference in fishing in an MPA versus outside of an MPA? \n \n Higher Abundance")+
  theme_bw() + 
  theme(panel.grid = element_blank(),
         plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        legend.position = "none", 
...
ss16b <- survey_dat_filter %>% select(Region, q17) %>% drop_na() %>% group_by(Region)%>%count(q17 = factor(q17))%>% 
    mutate(pct = n/sum(n)*100,
           pct = round(pct, digits = 0))%>%
  ggplot(aes(x = q17, y = pct, label = paste0(pct, "%"), fill=Region))+
  geom_col(position = "dodge", color="black")+
  geom_text(position = position_dodge(width = 0.9),
            vjust = -0.5,
            size = 4)+
...
ss16c <- survey_dat_filter %>% select(Region,q18) %>% drop_na() %>%group_by(Region)%>% count(q18 = factor(q18))%>% 
    mutate(pct = n/sum(n)*100,
           pct = round(pct, digits = 0))%>%
  ggplot(aes(x = q18, y = pct, label = paste0(pct, "%"), fill=Region))+
  geom_col(position = "dodge", color="black")+
  geom_text(position = position_dodge(width = 0.9),
            vjust = -0.5,
            size = 4)+
...
ss23R <- survey_dat_filter %>% select(Region,q26) %>% drop_na() %>% group_by(Region)%>%count(q26 = factor(q26))%>% 
    mutate(pct = n/sum(n)*100,
           pct = round(pct, digits = 0))%>%
  ggplot(aes(x = q26, y = pct, label = paste0(pct, "%"), fill=Region))+
  geom_col(position = "dodge", color="black")+
  scale_fill_manual(values=c("#2166ac","#fddbc7","#b2182b", "Region"))+
  geom_text(position = position_dodge(width = 0.9),
            vjust = -0.5,
  ...
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  ggtitle("Responses By Management Region")+
  theme(panel.grid = element_blank(),
         plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        legend.position = "none", 
        axis.title=element_text(size=12),
...
#Age Group is Q32. # of Trips is Q3, How often is Q2

Q32 - 18-24, 25-34, 35-33, 45-54, 55-64, 65+

Q2 - Every Year, Every Few Years, Infrequently (1-2 years only), Never

Q3- 1, 2-5, 6-10, 11-19, 20+


```{r}

#Managed by Age
ss23A <- survey_dat_filter %>% select(Age,q26) %>% drop_na() %>% group_by(Age)%>%count(q26 = factor(q26))%>% 
    mutate(pct = n/sum(n)*100,
           pct = round(pct, digits = 0))%>%
  ggplot(aes(x = q26, y = pct, label = paste0(pct, "%"), fill=Age))+
  geom_col(position = "dodge", color="black")+
  scale_fill_brewer(palette = "BrBG")+
  #scale_fill_manual(values=c("#2166ac","#fddbc7","#b2182b", "Region"))+
  geom_text(position = position_dodge(width = 0.9),
  ...
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  ggtitle("How well are California groundfish stocks managed? \n  Responses by age group")+
  theme(panel.grid = element_blank(),
         plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        legend.position = "none", 
        axis.title=element_text(size=12),
...
# Managed by Frequency of participation
ss23F <- survey_dat_filter %>% select(Freq,q26) %>% drop_na() %>% filter(Freq!="Never")%>%group_by(Freq)%>%count(q26 = factor(q26))%>% 
    mutate(pct = n/sum(n)*100,
           pct = round(pct, digits = 0))%>%
  ggplot(aes(x = q26, y = pct, label = paste0(pct, "%"), fill=Freq))+
  geom_col(position = "dodge", color="black")+
  #scale_fill_manual(values=c("#2166ac","#fddbc7","#b2182b", "Region"))+
  geom_text(position = position_dodge(width = 0.9),
            vjust = -0.5,
  ...
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  ggtitle("How well are California groundfish stocks managed? \n Frequency of Participation with CCFRP")+
  theme(panel.grid = element_blank(),
         plot.title = element_text(hjust = 0.5),
        legend.position = "none", 
        axis.title=element_text(size=12),
...
# Managed by Number of Trips 
ss23T <- survey_dat_filter %>% select(Trips,q26) %>% drop_na() %>% group_by(Trips)%>%count(q26 = factor(q26))%>% 
    mutate(pct = n/sum(n)*100,
           pct = round(pct, digits = 0))%>%
  ggplot(aes(x = q26, y = pct, label = paste0(pct, "%"), fill=Trips))+
  geom_col(position = "dodge", color="black")+
  #scale_fill_manual(values=c("#2166ac","#fddbc7","#b2182b", "Region"))+
  geom_text(position = position_dodge(width = 0.9),
            vjust = -0.5,
  ...
  labs(x = '', y = 'Percent\n') +
  theme_bw() + 
  ggtitle("How well are California groundfish stocks managed? \n Number of CCFRP trips attended")+
  theme(panel.grid = element_blank(),
         plot.title = element_text(hjust = 0.5),
        legend.position = "none", 
        axis.title=element_text(size=12),
...
Gender identity - Q31, Male, Female, Non-Binary, Prefer not to answer, other
Age Group -- Q32 - 18-24, 25-34, 35-33, 45-54, 55-64, 65+
Education - Q33 - Less than high school degree, High school degree or equivalent (e.g., GED), Associate degree or completion of techincal or professional school, Some college but no degree, Bachelor degree, Graduate degree (e.g., Master degree, PhD, JD, MD, etc,)
Income - Q34 - Less than $25,000, $25,00-$34,999, $35,000-$49,999, $50,000-$74,999, $75,000-$99,999, $100,000-$149,999 $150,000-$199,999, $200,000 or more, Decline to state

```{r}
gender <- c("Male", "Female", "Non-Binary", "Prefer not to answer")

ss31 <- survey_dat_clean %>% 
  select(q31) %>%
  filter(q31<5)%>%
  drop_na() %>%
  separate_rows(q31) %>% 
  count(q31 = factor(q31))%>% 
  mutate(pct = n/sum(!is.na(survey_dat_clean$q31))*100,
         pct = round(pct, digits = 1))%>%
  ...
  #labs(x = '\nWhat aspects of MPAs affected groundfish size/abundance?', y = 'Percent') +
  labs(x = 'Gender identity', y = 'Percent\n') +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        legend.position = "none", 
...
```{r q31 plot, fig.height= 6, fig.width= 9, fig.align= "center", echo = F}
...
```{r}
Age <- c("18-24", "25-34", "35-33", "45-54", "55-64", "65+")

ss32 <- survey_dat_clean %>% 
  select(q32) %>%
  drop_na() %>%
  separate_rows(q32) %>% 
  count(q32 = factor(q32))%>% 
  mutate(pct = n/sum(!is.na(survey_dat_clean$q32))*100,
         pct = round(pct, digits = 1))%>%
...
```{r q32 plot, fig.height= 6, fig.width= 9, fig.align= "center", echo = F}
...
```{r}
Education <- c("Less than \nhigh school degree", "High school \ndegree", "Associates degree \nor tech school", "Some college", "Bachelor degree", "Graduate degree")

ss33 <- survey_dat_clean %>% 
  select(q33) %>%
  drop_na() %>%
  separate_rows(q33) %>% 
  count(q33 = factor(q33))%>% 
  mutate(pct = n/sum(!is.na(survey_dat_clean$q33))*100,
         pct = round(pct, digits = 1))%>%
  ...
  scale_x_discrete(labels = Education) +
  #labs(x = '\nWhat aspects of MPAs affected groundfish size/abundance?', y = 'Percent') +
...
```{r q33 plot, fig.height= 6, fig.width= 9, fig.align= "center", echo = F}
...
ss34 <- survey_dat_clean %>% 
  select(q34) %>%
  drop_na() %>%
  separate_rows(q34) %>% 
  count(q34 = factor(q34))%>% 
  mutate(pct = n/sum(!is.na(survey_dat_clean$q34))*100,
         pct = round(pct, digits = 1))%>%
  ...
  scale_x_discrete(labels = Income) +
  #labs(x = '\nWhat aspects of MPAs affected groundfish size/abundance?', y = 'Percent') +
...
```{r q34 plot, fig.height= 6, fig.width= 9, fig.align= "center", echo = F}

ss34

```

```{r q31,32,33,34 plot, fig.height= 6, fig.width= 9, fig.align= "center", echo = F}
